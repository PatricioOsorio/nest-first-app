...
Este temario sintetiza los cursos oficiales de NestJS (*Fundamentals*, *Authentication*, *Architecture & Advanced Patterns*), la documentación oficial y los estándares de ingeniería de software para proyectos empresariales en producción.

Cada tema sigue el protocolo de [AGENTS.md](./AGENTS.md):
1. **Under the Hood**: Fundamentos técnicos, ciclo interno y paralelismos con C# / Angular.
2. **Hands-on Challenge**: Reto práctico que implementarás en el repositorio.
3. **Definition of Done (DoD)**: Criterios de aceptación y verificación.
...
- **Under the Hood**: Principio 12-Factor App para configuración; cómo `@nestjs/config` lee `.env`, y por qué la validación con esquema (`Joi` o `Zod`) debe detener el arranque de la app (*fail-fast*) si falta una variable requerida.
...
- **Under the Hood**: Comparativa entre estructura por capas (*layer-first*) vs por características (*feature-first*). Por qué aislar por features (`src/users`, `src/products`) junto a `src/common` o `src/core` mejora la escalabilidad y mantenibilidad.
...
- **Paralelismo**: `NgModule` en Angular; modularización por *bounded contexts*.
